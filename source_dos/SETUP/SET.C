...
#define MENU_SERSET         7
#define MENU_JSTICK         3
#define MENU_MOUSE          4
#define MENU_SAVEEXIT       8
#define MENU_SAVEEXITRUN    9
#define MENU_VRHELMET       5
#define TITLELEN            32
#define MAXITEMS            16
#define ITEMLEN             32
#define HELPLEN             60
...
//  "Save and Run Greed"
  },
 {
  ...
  "Configure Virtual Reality Helmet",
...
MENU VRHelmet =
{
 ...
 {
  "Don't use a VR Helmet",
  "Select SimulEyes VR Glasses by StereoGraphics",
  }
 };
...
/* select map */
void SelectMap(SoundCard *SC)
{
 selectmap4.Line=SC->netmap-22;
 if (ExecMenu(&selectmap4)>=0)
  SC->netmap=selectmap4.Line+22;
/*
 while (1)
  {
   if (ExecMenu(&selectmap0)<0) break;
   switch (selectmap0.Line)
    {
     case 0:
      if (ExecMenu(&selectmap1)>=0)
       SC->netmap=selectmap1.Line;
      break;
     case 1:
      if (ExecMenu(&selectmap2)>=0)
       SC->netmap=selectmap2.Line+8;
      break;
     case 2:
      if (ExecMenu(&selectmap3)>=0)
       SC->netmap=selectmap3.Line+16;
      break;
     case 3:
      if (ExecMenu(&selectmap4)>=0)
       SC->netmap=selectmap4.Line+22;
...
void sethelmet(SoundCard *SC)
{
 VRHelmet.Line=SC->vrhelmet;
 if (ExecMenu(&VRHelmet)>=0)
  SC->vrhelmet=VRHelmet.Line;
 }
 ...
 for (;;)
  {
   ...
   else if (MainMenu.Line == MENU_VRHELMET) sethelmet(SC);
