...
static Sample *LoadSample(int Handle, long Length)
{
    Sample *SampPtr;
    void *DataPtr;
    ...
    if (read(Handle,SampPtr->DataPtr,SampPtr->Length) != SampPtr->Length) {
        dError = ERR_FILEIO;
        free(DataPtr);
        free(SampPtr);
        return NULL;
...
static Pattern *LoadPattern(int Handle, long Length)
{
    Pattern *PattPtr;
...
DSM *dLoadModuleFile(int Handle, dword Length)
{
    RiffHeader Header;
    RiffBlock Block;
    DSM *Module;
    ...
    if (sizeof(Header) + Header.Length > Length) {
        dError = ERR_FORMAT;
        dFreeModule(Module);
        return NULL;
    }
    while (Header.Length) {
        ...
        if (Block.ID == ID_SONG) {
            if (read(Handle,&Module->Header,Block.Length) != Block.Length) {
                dError = ERR_FILEIO;
                dFreeModule(Module);
                return NULL;
            }
            ...
            if (!(Module->Samples[SampNum++] =
                    LoadSample(Handle,Block.Length))) {
                dFreeModule(Module);
                return NULL;
            }
        }
            ...
            if (!(Module->Patterns[PattNum++] =
                    LoadPattern(Handle,Block.Length))) {
                dFreeModule(Module);
                return NULL;
            }
        }
...
*               Flags   - sample data format
*
* Returns:      Sample structure containing the WAVE digital sample
*               or NULL when an error has occurred.
*
* Description:  Used to load RAW sample files.
*
****************************************************************************/

Sample *dLoadSampleFile(int Handle, dword Length, byte Flags)
{
    Sample *SampPtr;
    void *DataPtr;
...
DSM *dLoadModule(char *Filename)
{
    ...
    Module = dLoadModuleFile(Handle,filelength(Handle));
    close(Handle);
    return Module;
}
...
*               while loading the file.
*
* Description:  Load 8-bit signed RAW sample files from disk.
*
****************************************************************************/
...
Sample *dLoadSample(char *Filename)
{
    ...
    SampPtr = dLoadSampleFile(Handle,filelength(Handle),SF_8BITS|SF_SIGNED);
    close(Handle);
    return SampPtr;
}
