...
void MS_GetDOSMem(int size,int *seg,int *sel)
{
 union REGS r;

 r.w.ax=DOSGETMEM;
 r.w.bx=(size/16) + 1;  // number of paragraphs
 int386(DPMIINT,&r,&r);
 if (r.w.cflag)
  MS_Error("GEN_GetDOSMem: Error Getting DOS Memory");
 *seg=r.w.ax;
 *sel=r.w.dx;
 }
