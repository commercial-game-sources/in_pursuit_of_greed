...
Sample *dLoadSampleFile(int Handle, long Length)
{
    ...
    if (Wave.Hdr.ID!=ID_RIFF || Wave.Hdr.Type!=ID_WAVE ||
	    Wave.Fmt.ID!=ID_FMT || Wave.Data.ID!=ID_DATA ||
	    Wave.F.Format!=WAVE_FMT_PCM || Wave.F.Channels!=1 || Wave.F.BitsPerSample!=8)
     {
      dError=ERR_FORMAT;
      return NULL;
      }
    ...
    if (sizeof(RiffBlock)+Wave.Hdr.Length>Length)
     {
    ...
    if (read(Handle,DataPtr,SampPtr->Length)!=SampPtr->Length)
     {
      dError=ERR_FILEIO;
      free(DataPtr);
      free(SampPtr);
